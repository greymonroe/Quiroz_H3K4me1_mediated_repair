...
results<-rbindlist(lapply(normals, function(norm){
  ...
  mutations<-rbindlist(lapply(tumor, function(f){
    if(norm!=f){
      dat<-read.vcfR(paste0("data/strelka_out/",norm,"_",f,"/results/variants/somatic.indels.vcf.gz"), verbose = F)
...
fwrite(results, "data/strelka2_results_indels.csv")
...
PASS$tumor_ref_depth<-as.numeric(apply(PASS, 1, function(x) unlist(strsplit(unlist(strsplit(x["TUMOR"], split = ":"))[3],split=","))[1]))
PASS$tumor_alt_depth<-as.numeric(apply(PASS, 1, function(x) unlist(strsplit(unlist(strsplit(x["TUMOR"], split = ":"))[4],split=","))[1]))
...
PASS3$size<-nchar(PASS3$ALT)-nchar(PASS3$REF)
PASS3$mut<-ifelse(PASS3$size>0, paste0("+",substr(PASS3$ALT,nchar(PASS3$REF)+1,nchar(PASS3$ALT))), paste0("-",substr(PASS3$REF,nchar(PASS3$ALT)+1,nchar(PASS3$REF))))
...
fwrite(PASS3, "data/MSH6_KO_mutations_indels.csv")
